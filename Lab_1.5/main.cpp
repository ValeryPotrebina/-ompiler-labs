...
{CHAR} {
    ...
    std::string parsed = text.substr(1, text.length() - 2);
    wchar_t value;
    if(parsed[0] == '\\' && parsed.length() >= 2){
        switch (parsed[1]){
        case 'n':
            value = '\n';
            break;
        ...
        case '\'':
            ...
            break;
        default:
            const char* hex = (std::string("0x") + parsed.substr(1, parsed.length() - 1)).c_str();
            value = std::strtoul(hex, (char**)0, 0);
            break;
        }
    }else{
        //else set first symbol
        value = parsed[0];
    }
    yylval->value = value;
    return TAG_CHAR;
...
int main (int argc, char **argv) {
    ...
    while(1) {
        ...
        print_frag(&coords);
        switch(tag){
            case 1: printf(": (%d, %s)", value.code, codeName[value.code].c_str()); break;
            case 2: printf(": '%lc'", value.value); break;
            case 3:
            case 4:
            case 5: break;
        }
